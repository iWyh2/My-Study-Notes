...
- 属性是对**指定的对象**设置的样式属性，例如：字体大小、文本颜色等
- 属性和属性值之间用英文 `:` 分开
...
注意：`<style>` 标签可以写到其他标签内部并作用于该标签区域内，但是强烈不推荐这种写法！
...
- 在双引号中间，写法要符合 CSS 规范
- 可以控制当前的标签设置样式
- 由于书写繁琐，并且没有体现出结构与样式相分离的思想，所以不推荐大量使用，只有对当前元素添加简单样式的时候，可以考虑使用，配合JS使用
...
- `<style>` 标签理论上可以放在 HTML 文档的任何地方，但一般会放到文档的 `<head>` 标签中，`<title>`标签的下面
- 目前的浏览器已经支持**省略** `type` **属性**
- 通过此种方式，可以方便控制当前整个页面中的元素样式设置
- 代码结构清晰，但是并没有实现结构与样式完全分离
- 使用内部样式表设定 CSS，通常也被称为 `嵌入式引入`，这种方式是我们练习时常用的方式，用于做小案例
...
| `href` | 定义所链接外部样式表文件的 URL，可以是**相对路径**，也可以是绝对路径 |

**注意：**使用外部样式表设定 CSS，通常也被称为 `外链式` 或 `链接式引入`，这种方式是开发中常用的方式，用于做项目时。

## 5.5 CSS引入方式总结

| 样式表               | 优点                     | 缺点         | 使用情况       |    使用场景    | 控制范围     |
| -------------------- | ------------------------ | ------------ | -------------- | :------------: | ------------ |
| 行内样式表（行内式） | 书写方便，权重高         | 结构样式混写 | 较少           |   配合JS使用   | 控制一个标签 |
| 内部样式表（嵌入式） | 部分结构和样式分离       | 没有彻底分离 | 较多           | 做小案例练习时 | 控制一个页面 |
| 外部样式表（外链式） | 完全实现结构和样式相分离 | 需要引入     | 最多，吐血推荐 |   做项目开发   | 控制多个页面 |
...
如果想**要差异化选择不同的标签**，单独选一个或者某几个标签，可以使用 `类选择器` 。
...
- font-family的值有多个 按顺序有则使用这个字体 没有则依次使用 最后无衬线字体sans-serif兜底
...
**注意：**平时我们很少给文字加斜体，反而要**给斜体标签 (em、i) 改为不倾斜字体**。
...
给同一个标签设置了相同的样式，浏览器的渲染方式是：层叠覆盖。即写在最下面的会生效。后面的覆盖前面的。
...
例如：
body {
	font: normal 400 font-size "Microsoft YaHei", Arial, sans-serif;
}
```

- 使用 font 属性时，必须按上面语法格式中的顺序书写，不能更换顺序，并且各个属性间以空格隔开
- 不需要设置的属性可以省略（取默认值），但只能省略前两个，必须保留 font-size 和 font-family 属性，否则 font 属性将不起作用
...
`text-align` 属性只能**作用于 `块级元素`**，并**让该块级元素内的 `行内元素` 实现居中**（不一定是文字）。

* 文本
* span标签 a标签
* input标签 img标签

让以上元素要水平居中 则需要给他们的外层父级(外层的那个元素标签)设置` text-align:center`属性，比如:
...
<body>
    <h1>新闻标题</h1>
    <br />
    <img src="./mainBg.png" alt="美少女" />
</body>
```

上例中，让标题内容“新闻标题”居中 其外级元素标签为`<h1>`，则为其添加此属性，让图片居中，其外级元素标签为`<body>`，所以给其添加此属性
...
p {
	line-height: 2;
	指的是字体大小font-size的倍数的行高
}
```

注：font的复合属性中可以加入行高 `font: style weight size/line-height family` 

​		网页精准布局时，会使用`line-height：1` 取消上下间距
...
|  颜色表示方法  |               表示含义                |        属性值         |
| :------------: | :-----------------------------------: | :-------------------: |
|   rbg表示法    |   红绿蓝三原色，每项取值范围：0-255   |      rgb(0,0,0)       |
|   rgba表示法   | 红绿蓝三原色+a(透明度，取值范围：0-1) | rgba(255,255,255,0.3) |
| 十六进制表示法 |    #开头，将数字转换成十六进制表示    |        #000000        |
|    关键词法    |           直接输入颜色单词            |       red，blue       |



## 4.8 标签水平居中方法

让div p h水平居中 通过设置margin: 0 auto实现
...
注：开发中div使用很多 用来布局 则记住原则：用类名选择器区分
