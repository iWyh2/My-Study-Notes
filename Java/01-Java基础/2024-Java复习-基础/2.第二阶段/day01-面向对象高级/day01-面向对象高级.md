...
main方法也是一个类方法，java xxx命令去运行一个程序时，编译器是通过调用 类名.main(...)来运行的。

关于main方法的参数args：是通过命令行输入的参数，会被存入到args里，通过空格隔开每个参数即可分别传入args数组中。如 java xxx 参数1 参数2 参数3 ....，这些参数就会被存入args数组中。可以遍历这个args数组得到每个参数。
...
单例设计模式指的是一个类只有一个实例对象

单例模式可分为 饿汉式单例 和 懒汉式单例

饿汉式单例：

```java
public class A {
    private static A a = new A(); // 调用时已经创建好
    private A() {
