...
多态的注意事项：多态是对象，行为上的多态，Java中的属性（成员变量）不谈多态，也就是编译看左，运行也看左。正常下，编译看左，运行看右。
...
- 类的成员（成员变量、成员方法、构造器），抽象类都可以有。如下面代码
...
父类知道每个子类都要做某个行为，但每个子类要做的情况不一样，父类就定义成抽象方法，交给每个子类去重写实现。**设计这样的抽象类，是为了更好的支持多态**。
