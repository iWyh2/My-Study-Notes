...
![](https://www.leixue.com/uploads/2020/07/RocketMQ.png)
...
消息队列（Message Queue）是一种“先进先出”的数据结构
...
# 1.启动NameServer
#进入安装目录的bin目录下
nohup sh mqnamesrv &   					 #出现了nohup: 忽略输入并把输出追加到"nohup.out"错误，可以忽略这个错误。
# 2.查看启动日志
tail -f ~/logs/rocketmqlogs/namesrv.log  #日志的默认路径
```
...
# 1.启动Broker
#进入安装目录的bin目录下
nohup sh mqbroker -n localhost:9876 &    #出现了nohup: 忽略输入并把输出追加到"nohup.out"错误，可以忽略这个错误。
# 2.查看启动日志
...
准备两个窗口，都进入安装目录的bin目录下，充当生产者和消费者
...
- NameServer是一个几乎无状态节点，可集群部署，节点之间无任何信息同步。（直接启动多个）

- Broker部署相对复杂，**Broker分为Master与Slave（Master负责写，Slave负责读）一个Master可以对应多个Slave，但是一个Slave只能对应一个Master**，Master与Slave的对应关系通过**指定相同的BrokerName**，不同的BrokerId来定义，**BrokerId为0表示Master，非0表示Slave**。Master也可以部署多个。每个Broker与NameServer集群中的所有节点建立长连接，定时注册Topic信息到所有NameServer。（稍微复杂）
- Producer与NameServer集群中的其中一个节点（随机选择）建立长连接，定期从NameServer取Topic路由信息，并向提供Topic服务的Master建立长连接，且定时向Master发送心跳。Producer完全无状态，可集群部署。（直接启动多个）
- Consumer与NameServer集群中的其中一个节点（随机选择）建立长连接，定期从NameServer取Topic路由信息，并向提供Topic服务的Master、Slave建立长连接，且定时向Master、Slave发送心跳。Consumer既可以从Master订阅消息，也可以从Slave订阅消息，订阅规则由Broker配置决定。（直接启动多个）
...
每个Master配置一个Slave，有多对Master-Slave，HA采用同步双写方式，即**只有主备都写成功，才向应用返回成功**，这种模式的优缺点如下：
...
5. Consumer跟Producer类似，跟其中一台NameServer建立长连接，**获取当前订阅Topic存在哪些Broker上**，然后**直接跟Broker建立连接通道**，开始消费消息。

### 3.3.3 服务器环境

一般要两台Linux服务器，之后的操作两台Linux都要执行

（我使用一台Linux虚拟机，一台Windows主机，**Windows配置Host信息和配置环境变量自行解决**）

| **序号** | **IP**                 | **角色**                 | **架构模式**    |
| -------- | ---------------------- | ------------------------ | --------------- |
| 1        | 192.168.88.92          | nameserver、brokerserver | Master1、Slave2 |
| 2        | 本机Windows的vent8的ip | nameserver、brokerserver | Master2、Slave1 |
...
# 本机Linux
# nameserver
192.168.88.92 rocketmq-nameserver1
# broker
192.168.88.92 rocketmq-master1
192.168.88.92 rocketmq-slave2
# 本机Windows - vmware的vent8的ip192.168.88.1
# nameserver
192.168.88.1 rocketmq-nameserver2
# broker
192.168.88.1 rocketmq-master2
192.168.88.1 rocketmq-slave1
```
...
```bash
#set rocketmq
ROCKETMQ_HOME=/home/wyh/wyh/RocketMQ/rocketmq-all-4.4.0-bin-release
...
```bash
#文件路径不能重复，不然会启动两个broker时失败
#Linux下文件路径
#master1
mkdir /usr/local/rocketmq/store/a-broker                  #消息存储位置
mkdir /usr/local/rocketmq/store/a-broker/commitlog
mkdir /usr/local/rocketmq/store/a-broker/consumequeue
mkdir /usr/local/rocketmq/store/a-broker/index
#slave2
mkdir /usr/local/rocketmq/store/b-s-broker                  #消息存储位置
mkdir /usr/local/rocketmq/store/b-s-broker/commitlog
mkdir /usr/local/rocketmq/store/b-s-broker/consumequeue
mkdir /usr/local/rocketmq/store/b-s-broker/index

#Windows下文件路径为
#master2
D:\RocketMQ\store\b-broker\commitlog
D:\RocketMQ\store\b-broker\consumequeue
D:\RocketMQ\store\b-broker\index
#slave1
D:\RocketMQ\store\a-s-broker\commitlog
D:\RocketMQ\store\a-s-broker\consumequeue
D:\RocketMQ\store\a-s-broker\index
```
...
```sh
vi /home/wyh/wyh/RocketMQ/rocketmq-all-4.4.0-bin-release/conf/2m-2s-sync/broker-a.properties
```
...
#存储路径
storePathRootDir=/usr/local/rocketmq/store/a-broker
#commitLog 存储路径
storePathCommitLog=/usr/local/rocketmq/store/a-broker/commitlog
#消费队列存储路径存储路径
storePathConsumeQueue=/usr/local/rocketmq/store/a-broker/consumequeue
#消息索引存储路径
storePathIndex=/usr/local/rocketmq/store/a-broker/index
#checkpoint 文件存储路径
storeCheckpoint=/usr/local/rocketmq/store/a-broker/checkpoint
#abort 文件存储路径
abortFile=/usr/local/rocketmq/store/a-broker/abort
#限制的消息大小
...
```sh
vi /home/wyh/wyh/RocketMQ/rocketmq-all-4.4.0-bin-release/conf/2m-2s-sync/broker-b-s.properties
```
...
diskMaxUsedSpaceRatio=88
#存储路径
storePathRootDir=/usr/local/rocketmq/store/b-s-broker
#commitLog 存储路径
storePathCommitLog=/usr/local/rocketmq/store/b-s-broker/commitlog
#消费队列存储路径存储路径
storePathConsumeQueue=/usr/local/rocketmq/store/b-s-broker/consumequeue
#消息索引存储路径
storePathIndex=/usr/local/rocketmq/store/b-s-broker/index
#checkpoint 文件存储路径
storeCheckpoint=/usr/local/rocketmq/store/b-s-broker/checkpoint
#abort 文件存储路径
abortFile=/usr/local/rocketmq/store/b-s-broker/abort
#限制的消息大小
maxMessageSize=65536
#flushCommitLogLeastPages=4
#flushConsumeQueueLeastPages=2
...
```sh
D:\RocketMQ\rocketmq-all-4.4.0-bin-release\conf\2m-2s-sync\broker-b.properties
```
...
diskMaxUsedSpaceRatio=88
#存储路径
storePathRootDir=D:\RocketMQ\store\b-broker
#commitLog 存储路径
storePathCommitLog=D:\RocketMQ\store\b-broker\commitlog
#消费队列存储路径存储路径
storePathConsumeQueue=D:\RocketMQ\store\b-broker\consumequeue
#消息索引存储路径
storePathIndex=D:\RocketMQ\store\b-broker\index
#checkpoint 文件存储路径
storeCheckpoint=D:\RocketMQ\store\b-broker\checkpoint
#abort 文件存储路径
abortFile=D:\RocketMQ\store\b-broker\abort
#限制的消息大小
maxMessageSize=65536
#flushCommitLogLeastPages=4
#flushConsumeQueueLeastPages=2
...
```sh
D:\RocketMQ\rocketmq-all-4.4.0-bin-release\conf\2m-2s-sync\broker-a-s.properties
```
...
diskMaxUsedSpaceRatio=88
#存储路径
storePathRootDir=D:\RocketMQ\store\a-s-broker
#commitLog 存储路径
storePathCommitLog=D:\RocketMQ\store\a-s-broker\commitlog
#消费队列存储路径存储路径
storePathConsumeQueue=D:\RocketMQ\store\a-s-broker\consumequeue
#消息索引存储路径
storePathIndex=D:\RocketMQ\store\a-s-broker\index
#checkpoint 文件存储路径
storeCheckpoint=D:\RocketMQ\store\a-s-broker\checkpoint
#abort 文件存储路径
abortFile=D:\RocketMQ\store\a-s-broker\abort
#限制的消息大小
maxMessageSize=65536
#flushCommitLogLeastPages=4
#flushConsumeQueueLeastPages=2
...
分别在本机Linux和本机Windows启动NameServer

```bash
cd /home/wyh/wyh/RocketMQ/rocketmq-all-4.4.0-bin-release/bin
nohup sh mqnamesrv &
#Windows启动nameserver直接双击mqnamesrv.cmd启动
```

#### 2）启动Broker集群

* 在本机Linux上启动master1和slave2

master1：

```bash
cd /home/wyh/wyh/RocketMQ/rocketmq-all-4.4.0-bin-release/bin
nohup sh mqbroker -c /home/wyh/wyh/RocketMQ/rocketmq-all-4.4.0-bin-release/conf/2m-2s-sync/broker-a.properties &
```
...
```sh
cd /home/wyh/wyh/RocketMQ/rocketmq-all-4.4.0-bin-release/bin
nohup sh mqbroker -c /home/wyh/wyh/RocketMQ/rocketmq-all-4.4.0-bin-release/conf/2m-2s-sync/broker-b-s.properties &
```

* 在本机Windows上启动master2和slave1

  Windows上用**start**代替nohup

  在D:\RocketMQ\rocketmq-all-4.4.0-bin-release\bin目录下敲cmd进入命令行

master2

```sh
start mqbroker.cmd -c D:\RocketMQ\rocketmq-all-4.4.0-bin-release\conf\2m-2s-sync\broker-b.properties
```

slave1

```sh
start mqbroker.cmd -c D:\RocketMQ\rocketmq-all-4.4.0-bin-release\conf\2m-2s-sync\broker-a-s.properties
```
...
`RocketMQ`有一个对其扩展的开源项目[incubator-rocketmq-externals](https://github.com/apache/rocketmq-externals)，这个项目中有一个子模块叫`rocketmq-console`，这个便是管理控制台项目了，先将[incubator-rocketmq-externals](https://github.com/apache/rocketmq-externals)拉到本地，因为我们需要自己对`rocketmq-console`目录进行编译打包运行。
...
注意：打包前在```rocketmq-console```的application.properties中配置```namesrv```集群地址：

```properties
rocketmq.config.namesrvAddr=192.168.88.92:9876;192.168.88.1:9876
```

在target目录中cmd启动rocketmq-console：

```sh
java -jar rocketmq-console-ng-1.0.1.jar
#jar包的启动放在Linux虚拟机中进行，因为application.properties文件中的配置路径为Linux路径格式，Windows启动jar包会失败
#可以在Windows中编译打包，最后jar包上传到Linux中运行
```

启动成功后，我们就可以通过浏览器访问`http://192.168.88.92:8080`进入控制台界面了，如下图：
...
【注】RocketMQ报sendDefaultImpl call timeout问题解决：

```java
producer.send(msg,10000);//设置一下timeout即可，不设置超时时间，默认是3秒，而代码运行连接mq，用了3.6秒，所以就直接报超时异常了
```

#### 2）发送异步消息

异步消息通常**用在对响应时间敏感的业务场景**，即发送端不能容忍长时间地等待Broker的响应。
...
顺序消费的原理解析：在**默认**的情况下消息发送会采取Round Robin**轮询**方式**把消息发送到不同的queue(分区队列)**；而消费消息的时候从多个queue上拉取消息，这种情况发送和消费是不能保证顺序。但是如果**控制发送的顺序消息只依次发送到同一个queue中，消费的时候只从这个queue上依次拉取，则就保证了顺序**。当发送和消费参与的queue只有一个，则是全局有序；如果多个queue参与，则为分区有序，即相对每个queue，消息都是有序的。
...
public class OrderStep {
     ...
     * 生成模拟订单数据
     */
    public static final List<OrderStep> Orders = new ArrayList<>();
    static {
...
public class Producer {
    public static void main(String[] args) throws Exception {
        // 实例化消息生产者Producer
        DefaultMQProducer producer = new DefaultMQProducer("wyh-group-1");
        // 设置NameServer的地址
        producer.setNamesrvAddr(/*"192.168.88.92:9876;*/"192.168.88.1:9876");
        // 启动Producer实例
        producer.start();
        for (OrderStep order : OrderStep.Orders) {
            // 创建消息，并指定Topic，Tag和消息体
            Message message = new Message("OrderTest", "wyh-Order", order.toString().getBytes());
            // 发送消息
            /*
             * send方法参数说明：
             * ①：消息对象
             * ②：消息队列的选择器
             * ③：选择队列的业务标识（这里使用订单id）（也就是说，根据这个来选择队列）
             * ④：超时时间（电脑性能差，防止连接资源时超时导致出现异常）
             */
            SendResult sendResult = producer.send(message, new MessageQueueSelector() {
                 ...
                 * @param message 消息对象
                 * @param o 业务标识
                 * @return 根据业务标识而选择出来的队列
                 */
                @Override
                public MessageQueue select(List<MessageQueue> list, Message message, Object o) {
                    long OrderId = (long) o;
                    //用orderId取模找坐标，id一样，坐标就一样
                    long index = (OrderId / list.size()) % list.size();
                    return list.get((int)index);
                }
            }, order.getOrderId(), 10000);
            System.out.println(order.getOrderId() + "'s MessageQueue is " + sendResult.getMessageQueue().getQueueId());
        }
        // 如果不再发送消息，关闭Producer实例。
        producer.shutdown();
    }
...
public class Consumer {
    public static void main(String[] args) throws Exception {
        // 实例化消息生产者,指定组名
        DefaultMQPushConsumer consumer = new DefaultMQPushConsumer("wyh-group-1");
        // 指定NameServer地址信息.
        consumer.setNamesrvAddr("192.168.88.1:9876");
        // 指定消费者的消费模式：负载均衡 CLUSTERING | 广播模式 BROADCASTING（默认负载均衡）
        //consumer.setMessageModel(MessageModel.CLUSTERING);
        // 订阅Topic和Tag
        consumer.subscribe("OrderTest"/*Topic*/, "*"/*Tag*/);// "*"为订阅一个Topic下的全部tag（还有常见的监听某个存在消息的队列 tagA || tagB）
        // 注册消息监听器，重写回调函数，处理消息
...
比如电商里，提交了一个订单就可以**发送一个延时消息，1h后去检查这个订单的状态，如果还是未付款就取消订单释放库存**。
...
现在RocketMq并不支持任意时间的延时，需要**设置几个固定的延时等级，从1s到2h分别对应着等级1到18**
...
如果**消息的总长度可能大于4MB时，这时候最好把消息进行分割**
...
当出现报错【**The broker does not support consumer to filter message by SQL92**】

我们需要开启对propertyfilter的支持：如果是单broker模式，那么直接在conf目录下的broker.conf文件中添加
...
【如果是双主双从之类的，在相应的2m-2s-sync或者2m-2s-async中的broker-xxx.properties中添加】
...
```shell
/home/wyh/wyh/Zookeeper/zookeeper-cluster/zookeeper-1/bin/zkServer.sh start
/home/wyh/wyh/Zookeeper/zookeeper-cluster/zookeeper-2/bin/zkServer.sh start
/home/wyh/wyh/Zookeeper/zookeeper-cluster/zookeeper-3/bin/zkServer.sh start
```
...
```java
@EnableDubboConfiguration//使Dubbo的配置生效
@SpringBootApplication
...
### 2.3.4 Dubbo-Admin管理平台

将资料中的dubbo-admin.war放在Windows中的Tomcat的webapps中，启动Tomcat，访问localhost:80/dubbo-admin/即可

（我提前已经将资料中的dubbo-admin.war的dubbo.properties的注册中心地址改为了Linux地址）
...
- 文件系统
  ...
  目前业界较为常用的几款产品（RocketMQ/Kafka/RabbitMQ）均采用的是**消息刷盘至所部署虚拟机/物理机的文件系统来做持久化**（刷盘一般可以分为异步刷盘和同步刷盘两种模式）。消息刷盘为消息存储提供了一种高效率、高可靠性和高性能的数据持久化方式。除非部署MQ机器本身或是本地磁盘挂了，否则一般是不会出现无法持久化的故障问题。
...
通过使用mmap的方式，可以省去向用户态的内存复制，提高速度。这种机制**在Java中是通过MappedByteBuffer实现**的

**RocketMQ充分利用了上述特性，也就是所谓的“零拷贝”技术，提高消息存盘和网络发送的速度**。（跳过复制到用户态）
...
- 消息最大重试次数的设置对相同 Group ID 下的所有Consumer 实例有效。
...
* 创建数据文件夹`dataDir`
* 启动`BrokerStartup`,配置 程序启动参数【`broker.conf`的地址】和 环境变量【`ROCKETMQ_HOME`】
...
消息中间件的设计思路一般是**基于主题订阅发布的机制**，消息生产者（Producer）发送某一个主题到消息服务器，消息服务器负责将消息持久化存储，消息消费者（Consumer）订阅该兴趣的主题，消息服务器根据订阅信息（路由信息）将消息推送到消费者（Push模式）或者消费者主动向消息服务器拉去（Pull模式），从而实现消息生产者与消息消费者解耦。为了避免消息服务器的单点故障导致的整个系统瘫痪，通常会部署多台消息服务器共同承担消息的存储。那消息生产者如何知道消息要发送到哪台消息服务器呢？如果某一台消息服务器宕机了，那么消息生产者如何在不重启服务情况下感知呢？
...
解析配置文件，填充NameServerConfig、NettyServerConfig属性值，并**创建NamesrvController**
...
根据启动属性创建NamesrvController实例，并**初始化**该实例。NameServerController实例为NameServer核心控制器
...
批量消息发送是将同一个主题的多条消息一起打包发送到消息服务端，减少网络调用次数，提高网络传输效率。当然，并不是在同一批次中发送的消息数量越多越好，其判断依据是单条消息的长度，如果单条消息内容比较长，则打包多条消息发送会影响其他线程发送消息的响应时间，并且单批次消息总长度不能超过DefaultMQProducer#maxMessageSize（4M）。
